...
let resourceFileExecutableOk: FileResult, resourceFileExecutableBad: FileResult;
...
function setupFiles() {
	// TODO Replace execs with a func from utils
...

	{
		...
		const rootDir = resourceSubdirManRoot.name,
			outputPath = path.join(
				resourceSubdirManOutput.name,
				yamlToJsonExt(resourceFileManNonExistent.name)
			),
			manData: ManifestData = {
				sourceName: 'Some Non-Existent Manifest',
...

	{
		...
		const rootDir = resourceSubdirManRoot.name,
			outputPath = path.join(
				resourceSubdirManOutput.name,
				yamlToJsonExt(resourceFileManNoNameAttribute.name)
			),
			manData: ManifestData = {
				sourceName: '',
				baseDirectory: rootDir,
